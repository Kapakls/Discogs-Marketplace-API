...
from library.models import Album, DiscogsMarketplaceListing
...
class MusicService:
    ...
    def __init__(
        ...
        self.spotify_service = (
            spotify_service
            or create_spotify_service()
        )

        self.discogs_service = (
            discogs_service
            or DiscogsService()
        )
    ...
    async def get_users_music(
        ...
        spotify_albums = await self.spotify_service.get_saved_albums(
            access_token
        )
        ...
        for spotify_album in spotify_albums:
            ...
            marketplace_listings = (
                self.discogs_service.search_marketplace(album)
            )
    ...
    def _save_album(
        ...
        if len(title) > 200:
            print(
                f"Skipping album with excessively long title: "
    ...
    def _save_listings(
        ...
        for marketplace_listing in marketplace_listings:
            ...
            listing, _ = (
                DiscogsMarketplaceListing.objects.update_or_create(
    ...
    def _parse_release_date(self, spotify_album):
